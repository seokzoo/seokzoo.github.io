...
$$\begin{flalign}q_\pi(s,a)&=\mathbb{E}_b[\rho\cdot G_t\mid S_t=s,A_t=a]\\&=\mathbb{E}_b[\frac{\prod_{i=1}^{T-t}\lbrace b(A_{t+i-1}\mid S_{t+i-1})p(S_{t+i}\mid A_{t+i-1},S_{t+i-1})\rbrace}{\prod_{i=1}^{T-t}\lbrace\pi(A_{t+i-1}\mid S_{t+i-1})p(S_{t+i}\mid A_{t+i-1},S_{t+i-1})\rbrace}\cdot G_t\mid S_t=s,A_t=a]\\&=\mathbb{E}_b[\frac{\prod_{i=1}^{T-t}b(A_{t+i-1}\mid S_{t+i-1})}{\prod_{i=1}^{T-t}\pi (A_{t+i-1}\mid S_{t+i-1})}\cdot G_t\mid S_t=s,A_t=a] \end{flalign}$$
