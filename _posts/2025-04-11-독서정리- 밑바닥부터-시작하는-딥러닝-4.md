...
이를 해결하기 위해서 TD method(Temporal Difference method)을 사용한다. TD를 기반으로 한 학습 방법에는 SARSA와 Q-learning이 있다. SARSA는 Bellman equation을 바탕으로 하고 Q-learning은 Bellman optimality equation을 바탕으로 한다. 먼저 TD method가 무엇인지 설명하겠다.
...
다음은 Q-learning이다. Q-learning은 Bellman optimality equation을 바탕으로 하며 off-policy로 학습이 진행된다. behavior policy는 아무것이나 사용해도 좋다.
