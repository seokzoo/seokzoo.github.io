...
$$ P(X*n,X*{n-1},\dots,X*1)=\prod*{i=1}^nP(X*i|\bigcap^{i-1}*{j=1} X_j)$$
...
$$\begin{aligned} Av=\lambda v\ (v\ne \vec{0})\\(A-I\lambda)v=0\\ det(A-I\lambda)=0 \end{aligned}$$
...
$$ D\_{KL}(P\Vert Q)=\mathbb{E}[\Delta I_i]=\mathbb{E}[\log p_i-\log q_i]$$

$$=\sum_x p_i\log_2(\frac{p_i}{q_i})$$
...
$$ x*{n+1}=x_n-J^{-1}\vert*{x=x_n} f(x_n)$$
...
$$ H*f=\begin{bmatrix}H*{f*0}\\H*{f*1}\\\vdots\\H*{f\_{m-1}} \end{bmatrix}$$
...
$$ x*{n+1}=x*{n}-\eta f^{\prime}(x_n)$$

$$ x*{n+1}=x*{n}-\eta \nabla f(x_n)$$

- Taylor series approximation을 고려한 Newton's method

$$\begin{aligned}f(x)\approx f(a)+f^{\prime}(a)(x-a)+\frac{1}{2}f^{\prime\prime}(a)(x-a)^2\\f(x+\Delta x)\approx f(x)+f^{\prime}(x)\Delta x+\frac{1}{2}f^{\prime\prime}(x)(\Delta x)^2\\\frac{d}{d(\Delta x)}f(x+\Delta x)\approx f^{\prime}(x)+f^{\prime\prime}(x)\Delta x\\f^{\prime}(x+\Delta x)=0,\ \Delta x=-\frac{f^{\prime}(x)}{f^{\prime\prime}(x)} \\x_{n+1}=x_n-\frac{f^{\prime}(x_n)}{f^{\prime\prime}(x_n)}\\x_{n+1}=x_n-H_f^{-1}\vert_{x=x_n}\nabla f(x_n) \end{aligned}$$
...
$$\begin{aligned} L(\theta;x,y)=L(w_1,w_2,\dots,w_n,b_1,b_2,\dots,b_m;x,y)\\\nabla L(\theta;x,y)\end{aligned}$$

- MSE(mean squared error)

$$ \text{MSE}=\frac{1}{n}\sum(y-y\_{\text{pred}})^2$$
...
$$\begin{aligned} \frac{\partial}{\partial x}(Wx+b)=W^{\top}\\\frac{\partial }{\partial W}(Wx+b)=x^{\top}\\\frac{\partial }{\partial b}(Wx+b)=1 \end{aligned}$$
...
$$\begin{aligned}W\leftarrow W-\eta \nabla W\\x\leftarrow x-\eta\frac{\partial f}{\partial x} \end{aligned}$$
...
$$\begin{aligned}v\leftarrow\mu v_{old}-\eta \frac{\partial f}{\partial w}\\w\leftarrow w+v \end{aligned}$$

- Nesterov momentum : 수렴이 좀 더 빠를 수도 있다

$$\begin{aligned}v\leftarrow \mu v_{old}-\eta \nabla f(w+\mu v_{old})\\ w\leftarrow w+v\end{aligned}$$
...
$$\begin{aligned}m\leftarrow \gamma m_{old}+(1-\gamma)(\frac{\partial f}{\partial w})^2\\ v \leftarrow - \frac{\eta}{\sqrt{m}} \frac{\partial f}{\partial w}\\ x\leftarrow x+v \end{aligned}$$

- RMSprop with Nesterov momentum

$$\begin{aligned}m\leftarrow \gamma m_{old}+(1-\gamma)(\nabla f(w+\mu v))^2\\ v \leftarrow \mu v_{old} -\frac{\eta}{\sqrt{m}} \nabla f(w+\mu v)\\ w\leftarrow w+v \end{aligned}$$

- AdaGrad : $w_i$는 $w$의 각 성분에 모두 적용한다는 의미이고 $\tau$는 이전까지의 모든 step이다

$$\begin{aligned} v_i\leftarrow \frac{-\eta}{\sqrt{\sum_{\tau} (\frac{\partial f}{\partial w_i})^2}}\frac{\partial f}{\partial w_i}\\ w\leftarrow w+v\end{aligned}$$
...
$$\begin{aligned}m\leftarrow \beta_1m+(1-\beta_1)\frac{\partial f}{\partial w}\\ v\leftarrow \beta_2v+(1-\beta_2)(\frac{\partial f}{\partial w})^2\\ \hat{m}\leftarrow \frac{m}{1-\beta_1^t}\\\hat{v}\leftarrow\frac{v}{1-\beta_2^t}\\ w\leftarrow w-\frac{\eta}{\sqrt{\hat{v}}+\epsilon}\hat{m} \end{aligned}$$
