---
date: 2026-03-19 00:00:00 +0900
layout: post
title: 지식정리 - 반도체에 대하여
categories: 지식정리
---

# Contents

- TOC
  {:toc}

# 반도체에 대하여
- 최외각 전자가 4개인 silicon에 불순물을 집어 넣어서 자유 전자(N형) 또는 전공(P형)을 만들어줌
- 다이오드 : P형 반도체와 N형 반도체를 접합시킨 것 (맞 닿아 있는 곳은 전기적으로 중성이 되어 공핍 영역이 되는데 이곳은 전자가 흐르지 않음. 단 N형 반도체 쪽에 -전극을 연결하고 P형 반도체 쪽에 +전극을 연결하면 전기가 흐르게 됨)
- 트랜지스터 : NPN처럼 P형 반도체와 N형 반도체 3개를 접합한 것 (한 쪽의 전압이 높아지면 다른 한쪽에 전류가 흐르게 됨)
- MOSFET은 트랜지스터를 실리콘 위에 만들기 쉽게한 형태임
	- MOSFET의 발전사 : Planer FET -> FinFET -> GAAFET -> MBCFET  


# 반도체 공정 과정 (8대 공정)

## 웨이퍼

실리콘 잉곳을 만들어서 얇게 잘라 silicon wafer를 만들어 낸다. 이때, 실리콘에 불순물을 만들어 P 또는 N형 반도체를 만들어 낼 수 있다.  

## 산화 공정

실리콘 겉 부분을 산화시켜서 산화막을 만들어 낸다.  

## 포토 공정

감광액을 도포한 후에 EUV 광선을 mask(reticle)에 반사 시킨 후 wafer에 전사한다.  

## 식각 공정(Etching)

포토 공정에서 새겨진 패턴에 따라 제거되어야 할 부분을 식각을 통해 제거한다.  

## 증착 공정 & 이온 주입

실리콘을 증착하거나 이온을 주입하는 과정이다. 이온을 주입하는 과정에서 P 또는 N형 반도체가 만들어 질 수 있다.  

## 금속 배선 공정

산화 공정 및 포토 공정을 통해 금속이 배선될 곳을 배치하고 금속을 도표하여 금속 배선을 만들어 내는 공정이다.  

## 테스트 공정

EDS 공정 같은 반도체의 전기적 특성을 검사하는 테스트를 통해 양품이 맞는지 확인하는 과정이다.  

## 패키징 공정

반도체를 포장하는 과정이라고 생각하면 된다. 본딩 공정에 의해 반도체가 회로(PCB)와 어떻게 신호를 주고 받을지 결정이 된다.  


# 반도체 Front-end 공정 과정 (PTEDC)


## 포토(Photo)


## 식각(Etch)


## 확산(Diffusion)


## 박막(Thin Film)


## C&C(Cleaning & CMP)


# Back-end

## EDS(Electrical Die Sorting)

- ET Test & WBI(Wafer Burn In)
	- Package Test
		- Burn in test
		- Core test
		- Speed test
	- Module Test
		- Elec test : 모듈 환경에서 테스트
		- 실장 test : 실제 장비에서 테스트
- Hot/Cold Test
- Repair/Final Test
- Inking

# NAND vs DRAM

- DRAM : 층은 한 층인데 layer는 50층 정도
	- DRAM은 셀 하나하나를 컨트롤할 수 있게 구조가 되어 있어서 lithography가 중요함
	- 물론 ALD/CVD 같은 deposition도 capacitor를 만드는데 중요함
- NAND : 100-200층 정도이고 layer도 50층 정도
	- NAND는 산화막과 질화막을 번갈아 가면서 deposition을 여러 번 해놓고 한 번에 깊게 etching을 해서 만듦 (photo 공정은 etching 할 때 위치를 잡아주거나 회로 그리는 정도에 사용)
- HBM
	- FEOL 과정에서 웨이퍼 위에 트랜지스터를 만들고, BEOL 과정에서 금속 배선으로 트랜지스터끼리를 연결 -> TSV 공정을 통해서 칩 다이를 관통하는 via를 만들어냄(1000-2000개)
	- HBM3 : 만들어진 다이를 micro-bump로 연결
	- HBM4 : 만들어진 다이를 hybrid-bonding으로 연결